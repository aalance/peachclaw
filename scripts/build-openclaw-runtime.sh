...
NODE
# Channel-entry smoke is a verification step (not a build artifact). It symlinks
# the source into a temp node_modules, which fails with EPERM on Windows hosts
# without Developer Mode / admin. Don't abort the embedded build over it.
node scripts/test-built-bundled-channel-entry-smoke.mjs \
  || echo "[openclaw-runtime] WARNING: channel-entry smoke skipped (symlink not permitted on this host)"
