...
import type { MossTtsConfig } from '../../shared/tts/constants';
...
interface IElectronAPI {
  ...
  tts: {
    ...
    ping: (cfg: MossTtsConfig) => Promise<{ ok: boolean; status?: number; error?: string }>;
    savePromptAudio: (
      dataBase64: string,
  ...
  pet: {
    showWorkbench: () => Promise<{ success: boolean }>;
    toggleMouseIgnore: () => Promise<boolean>;
    getMouseIgnore: () => Promise<boolean>;
    dragStart: () => void;
    dragMove: () => void;
    closeApprovalWindow: () => Promise<{ success: boolean }>;
    getModelImage: () => Promise<{ success: boolean; dataUrl?: string; error?: string }>;
    setVisible: (visible: boolean) => Promise<{ success: boolean; visible: boolean }>;
    getVisible: () => Promise<{ visible: boolean; enabled: boolean }>;
    onVisibilityChanged: (callback: (state: { visible: boolean }) => void) => () => void;
    notifyAppearanceChanged: () => Promise<{ success: boolean }>;
