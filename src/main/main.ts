...
import { buildMossRequest, type MossTtsConfig, mossHealthUrl, TtsIpc, validateMossBaseUrl } from '../shared/tts/constants';
...
let petMouseIgnore = false;
// While the user drags the pet by its body, remember where the window and the
// cursor were when the drag began so we can reposition by the cursor delta.
let petDragOrigin: { winX: number; winY: number; cursorX: number; cursorY: number } | null = null;
...
if (!gotTheLock) {
  ...
  // MOSS-TTS-Nano proxy. The renderer loads from localhost:5175, so a direct
  // fetch to the MOSS server on another port would be blocked by CORS; we do the
  // HTTP call here in the main process (Node global fetch, no CORS) and hand the
  // audio bytes back to the renderer as base64 for playback. Targets the demo
  // server's POST /api/generate (multipart form) which returns JSON audio_base64.
  ipcMain.handle(TtsIpc.Synthesize, async (_event, text: string, cfg: MossTtsConfig) => {
    if (!validateMossBaseUrl(cfg?.baseUrl)) {
      return { ok: false, error: 'invalid MOSS server URL' };
    }
    ...
    try {
      const plan = buildMossRequest(clean, cfg);
      const form = new FormData();
      for (const [key, value] of Object.entries(plan.fields)) form.append(key, value);
      // A user-supplied reference recording clones that voice instead of using
      // one of the server's presets.
      if (cfg.promptAudioPath) {
        try {
          const audio = await fs.promises.readFile(cfg.promptAudioPath);
          form.append(
            'prompt_audio',
            new Blob([audio]),
            path.basename(cfg.promptAudioPath) || 'reference.wav',
          );
        } catch {
          return { ok: false, error: `cannot read reference audio: ${cfg.promptAudioPath}` };
        }
      }
      ...
      // MOSS returns JSON: { audio_base64, sample_rate, ... } on success, or
      // { error } with a 4xx/5xx on failure.
      let data: { audio_base64?: string; error?: string } | null = null;
      try {
        data = (await res.json()) as { audio_base64?: string; error?: string };
      } catch {
        data = null;
      }
      if (!res.ok || !data) {
        const detail = data?.error ? `: ${data.error}` : '';
        return { ok: false, error: `MOSS server responded ${res.status} ${res.statusText}${detail}` };
      }
      if (data.error) return { ok: false, error: data.error };
      if (!data.audio_base64) return { ok: false, error: 'MOSS server returned no audio' };
      return { ok: true, audioBase64: data.audio_base64, mime: 'audio/wav' };
    } catch (error) {
      const aborted = error instanceof Error && error.name === 'AbortError';
      return { ok: false, error: aborted ? 'MOSS request timed out' : String(error) };
    } finally {
      clearTimeout(timer);
    }
  });

  // Lightweight readiness probe (GET /health) for the settings "test connection" button.
  ipcMain.handle(TtsIpc.Ping, async (_event, cfg: MossTtsConfig) => {
    if (!validateMossBaseUrl(cfg?.baseUrl)) {
      return { ok: false, error: 'invalid MOSS server URL' };
    }
    const controller = new AbortController();
    ...
    try {
      const res = await fetch(mossHealthUrl(cfg.baseUrl), { method: 'GET', signal: controller.signal });
      // Any HTTP response means the server is up and reachable.
      return { ok: true, status: res.status };
    } catch (error) {
      const aborted = error instanceof Error && error.name === 'AbortError';
      return { ok: false, error: aborted ? 'connection timed out' : String(error) };
    } finally {
      clearTimeout(timer);
    }
  });

  // Persist a converted reference recording for MOSS voice cloning. The renderer
  // decodes whatever the user picked (mp3/m4a/…) with the Web Audio API and hands
  // us plain WAV bytes, because the MOSS server can only read WAV/FLAC.
  ipcMain.handle(TtsIpc.SavePromptAudio, async (_event, dataBase64: string) => {
    try {
      if (typeof dataBase64 !== 'string' || !dataBase64) {
        return { ok: false, error: 'no audio data' };
      }
      const dir = path.join(app.getPath('userData'), 'pet-voice');
      await fs.promises.mkdir(dir, { recursive: true });
      const target = path.join(dir, 'reference.wav');
      await fs.promises.writeFile(target, Buffer.from(dataBase64, 'base64'));
  ...
  ipcMain.handle(PetIpcChannel.ShowWorkbench, () => {
    focusMainWindowForReason('pet window request');
    return { success: true };
  });

  ipcMain.handle(PetIpcChannel.ToggleMouseIgnore, () => {
    petMouseIgnore = !petMouseIgnore;
    if (petWindow && !petWindow.isDestroyed()) {
      petWindow.setIgnoreMouseEvents(petMouseIgnore, { forward: true });
    }
    return petMouseIgnore;
  });

  ipcMain.handle(PetIpcChannel.GetMouseIgnore, () => {
    return petMouseIgnore;
  });

  // Custom window drag for the pet: the sprite can't use -webkit-app-region:drag
  // (it would swallow the hover events the menu relies on), so the renderer
  // signals drag start/move and we reposition the window by the cursor delta.
  ipcMain.on(PetIpcChannel.DragStart, () => {
    if (!petWindow || petWindow.isDestroyed()) {
      petDragOrigin = null;
      return;
    }
    ...
    petDragOrigin = { winX, winY, cursorX: cursor.x, cursorY: cursor.y };
  });

  ipcMain.on(PetIpcChannel.DragMove, () => {
    if (!petDragOrigin || !petWindow || petWindow.isDestroyed()) return;
    const cursor = screen.getCursorScreenPoint();
    petWindow.setPosition(
      petDragOrigin.winX + (cursor.x - petDragOrigin.cursorX),
      petDragOrigin.winY + (cursor.y - petDragOrigin.cursorY),
    );
  });

  ipcMain.handle(PetIpcChannel.CloseApprovalWindow, () => {
    if (approvalWindow && !approvalWindow.isDestroyed()) {
      approvalWindow.hide();
    }
    return { success: true };
  ...
  const broadcastPetVisibility = (visible: boolean) => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send(PetIpcChannel.VisibilityChanged, { visible });
      }
    }
  };

  ipcMain.handle(PetIpcChannel.SetVisible, (_event, payload: { visible?: boolean } | undefined) => {
    const visible = payload?.visible !== false;
    getStore().set('pet_visible', visible);
    if (visible) {
      createPetWindow();
    } else if (petWindow && !petWindow.isDestroyed()) {
      petWindow.hide();
    }
    broadcastPetVisibility(visible);
    return { success: true, visible };
  });

  ipcMain.handle(PetIpcChannel.GetVisible, () => {
    const stored = getStore().get<boolean>('pet_visible');
    const persisted = stored !== false; // default true
    const shown = Boolean(petWindow && !petWindow.isDestroyed() && petWindow.isVisible());
    return { visible: persisted && shown, enabled: persisted };
  });

  // Workbench saved a new pet appearance — tell the live pet window to reload.
  ipcMain.handle(PetIpcChannel.NotifyAppearanceChanged, () => {
    if (petWindow && !petWindow.isDestroyed()) {
      petWindow.webContents.send(PetIpcChannel.AppearanceChanged);
    }
    return { success: true };
  });

  ipcMain.handle(PetIpcChannel.GetModelImage, async () => {
    // Prefer the sprite bundled with the app (resources/pets/peach) so a fresh
    // clone or packaged build works on any machine. Fall back to the original
    // authoring copy under ~/.codex so the source dev machine still works.
    const bundledPath = app.isPackaged
      ? path.join(process.resourcesPath, 'pets', 'peach', 'spritesheet.webp')
      : path.join(__dirname, '..', 'resources', 'pets', 'peach', 'spritesheet.webp');
    const legacyPath = path.join('C:', 'Users', '29292', '.codex', 'pets', 'peach', 'spritesheet.webp');
    const candidates = [bundledPath, legacyPath];
    for (const spritePath of candidates) {
      try {
        const buffer = await fs.promises.readFile(spritePath);
        return { success: true, dataUrl: `data:image/webp;base64,${buffer.toString('base64')}` };
      } catch {
        // Try the next candidate location.
      }
    }
    const message = `pet model image not found (looked in: ${candidates.join(', ')})`;
    console.warn('[Pet]', message);
  ...
  // 创建主窗口
  const loadRendererWindow = (win: BrowserWindow, query: string): void => {
    if (isDev) {
      const url = new URL(DEV_SERVER_URL);
      url.search = query;
      win.loadURL(url.toString()).catch(error => {
        console.error('[Main] failed to load renderer window:', error);
      });
      return;
    }

    win.loadFile(path.join(__dirname, '../dist/index.html'), { search: query }).catch(error => {
  ...
  const createPetWindow = () => {
    ...
    loadRendererWindow(petWindow, '?window=pet');
  };

  const createApprovalWindow = () => {
    if (approvalWindow && !approvalWindow.isDestroyed()) {
      return;
    }
    ...
    loadRendererWindow(approvalWindow, '?window=approval');
  };
  ...
  const createWindow = () => {
    ...
    mainWindow.once('ready-to-show', () => {
      // 桌宠优先模式：启动时不自动显示 OpenClaw 主窗口，仅显示桌宠 + 托盘图标。
      // 用户可通过桌宠的"工作台"按钮或托盘菜单随时打开主窗口。
      // （开机自启同样不显示窗口。）
      // if (!isAutoLaunched()) {
      //   mainWindow?.show();
      // }
