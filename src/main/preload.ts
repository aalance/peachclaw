...
import { type MossTtsConfig, TtsIpc } from '../shared/tts/constants';
...
contextBridge.exposeInMainWorld('electron', {
  ...
  tts: {
    synthesize: (text: string, cfg: MossTtsConfig) =>
      ipcRenderer.invoke(TtsIpc.Synthesize, text, cfg),
    ping: (cfg: MossTtsConfig) => ipcRenderer.invoke(TtsIpc.Ping, cfg),
    savePromptAudio: (dataBase64: string) =>
      ipcRenderer.invoke(TtsIpc.SavePromptAudio, dataBase64),
  },
  skills: {
  ...
  pet: {
    showWorkbench: () => ipcRenderer.invoke(PetIpcChannel.ShowWorkbench),
    toggleMouseIgnore: () => ipcRenderer.invoke(PetIpcChannel.ToggleMouseIgnore),
    getMouseIgnore: () => ipcRenderer.invoke(PetIpcChannel.GetMouseIgnore),
    dragStart: () => ipcRenderer.send(PetIpcChannel.DragStart),
    dragMove: () => ipcRenderer.send(PetIpcChannel.DragMove),
    closeApprovalWindow: () => ipcRenderer.invoke(PetIpcChannel.CloseApprovalWindow),
    getModelImage: () => ipcRenderer.invoke(PetIpcChannel.GetModelImage),
    setVisible: (visible: boolean) => ipcRenderer.invoke(PetIpcChannel.SetVisible, { visible }),
    getVisible: () => ipcRenderer.invoke(PetIpcChannel.GetVisible),
    onVisibilityChanged: (callback: (state: { visible: boolean }) => void) => {
      const handler = (_event: any, state: { visible: boolean }) => callback(state);
      ipcRenderer.on(PetIpcChannel.VisibilityChanged, handler);
      return () => ipcRenderer.removeListener(PetIpcChannel.VisibilityChanged, handler);
    },
    notifyAppearanceChanged: () => ipcRenderer.invoke(PetIpcChannel.NotifyAppearanceChanged),
    onAppearanceChanged: (callback: () => void) => {
      const handler = () => callback();
      ipcRenderer.on(PetIpcChannel.AppearanceChanged, handler);
      return () => ipcRenderer.removeListener(PetIpcChannel.AppearanceChanged, handler);
    },
  },
